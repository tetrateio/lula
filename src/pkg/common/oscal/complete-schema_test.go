...
func TestInjectIntoOSCALModel(t *testing.T) {
	// Load test data
	targetCompDefBytes := loadTestData(t, "../../../test/unit/common/oscal/valid-component.yaml")
	compDefMetadataBytes := loadTestData(t, "../../../test/unit/common/oscal/valid-component-metadata.yaml")
	expectedCompDefInjectedBytes := loadTestData(t, "../../../test/unit/common/oscal/valid-component-metadata-injected.yaml")

	// Convert the test data to expected types
	var targetCompDef oscalTypes_1_1_2.OscalModels
	if err := yaml.Unmarshal(targetCompDefBytes, &targetCompDef); err != nil {
		t.Fatalf("yaml.Unmarshal failed: %v", err)
	}
	var compDefMetadata map[string]interface{}
	if err := yaml.Unmarshal(compDefMetadataBytes, &compDefMetadata); err != nil {
		t.Fatalf("yaml.Unmarshal failed: %v", err)
	}
	var expectedCompDefInjected oscalTypes_1_1_2.OscalModels
	if err := yaml.Unmarshal(expectedCompDefInjectedBytes, &expectedCompDefInjected); err != nil {
		t.Fatalf("yaml.Unmarshal failed: %v", err)
	}
