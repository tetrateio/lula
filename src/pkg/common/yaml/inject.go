...
func InjectMapData(target, subset map[string]interface{}, path string) (map[string]interface{}, error) {
	...
	// Merge the subset into the target's subset
	// newNode, err := merge2.Merge(subsetNode, targetSubsetNode, yaml.MergeOptions{ListIncreaseDirection: yaml.MergeOptionsListAppend})
	// if err != nil {
	// 	return nil, fmt.Errorf("error merging subset into target: %v", err)
	// }
	// Set the merged node back into the target node
	// if err := targetNode.PipeE(yaml.Lookup(pathSlice[:len(pathSlice)-1]...), yaml.SetField(pathSlice[len(pathSlice)-1], newNode)); err != nil {
	// 	return nil, fmt.Errorf("error setting merged node back into target: %v", err)
	// }
	// if len(pathSlice) == 1 {
	// 	if err := targetNode.PipeE(yaml.SetField(pathSlice[len(pathSlice)-1], newNode)); err != nil {
	// 		return nil, fmt.Errorf("error setting merged node back into target: %v", err)
	// 	}
	// } else {
	// 	if err := targetNode.PipeE(yaml.Lookup(pathSlice[:len(pathSlice)-1]...), yaml.SetField(pathSlice[len(pathSlice)-1], newNode)); err != nil {
